...
import { Server, Layers, Activity, Search, PlusCircle, Database, Cpu } from 'lucide-react';

export default function Lab() {
    const {
        blocks, mempool, logs, difficulty, setDifficulty,
        submitOperation, mineManual, isMining, getOperationDetails,
        latency, tps, trigger51Attack
    } = useSimulation();
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '24px' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', borderLeft: '1px solid #e2e8f0', paddingLeft: '24px' }}>
                        <div className="heartbeat" style={{
                            width: '12px', height: '12px', borderRadius: '50%', backgroundColor: '#10b981'
                        }}></div>
                        <span style={{ fontWeight: 600, fontSize: '0.9rem', color: '#10b981' }}>NETWORK LISTENING</span>
                    </div>
                </div>
            </header>
                ...
                <section style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                            ...
                            {blocks.slice().reverse().map(block => (
                                <div key={block.hash} style={{
                                    ...
                                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                                        <span style={{ fontWeight: 'bold' }}>BLOCK #{block.index} [ID: {block.blockId}]</span>
                                    ...
                                    <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                                        ...
                                        {block.operations.length === 0 && <span style={{ fontSize: '0.75rem', color: '#94a3b8' }}>Empty Block</span>}
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    <div className="card">
                        ...
                        <div style={{ display: 'flex', gap: '8px', marginBottom: searchResult ? '1rem' : '0' }}>
                            <input
                                placeholder="Enter Operation ID (e.g. 7X2...)"
                                value={searchId}
                        ...
                        {searchResult && (
                            <div style={{ background: '#f1f5f9', padding: '12px', borderRadius: '4px', fontSize: '0.85rem' }}>
                                ...
                                <div style={{ marginBottom: '4px' }}><strong>Creator:</strong> {searchResult.op.creator}</div>
                        ...
                        {searchResult === null && searchId && <div style={{ fontSize: '0.8rem', color: '#ef4444', marginTop: '8px' }}>Operation not found.</div>}
                    </div>
